...
# This file is called from the "fit.ouf.R" file; don't need to run directly
...
if (p == 1){
  ...
  lmm2 <- lme(Y2 ~ eta1 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
  lmm3 <- lme(Y3 ~ eta1 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
  lmm4 <- lme(Y4 ~ eta1 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
}else if (p == 2){
  lmm1 <- lme(Y1 ~ eta1 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
  ...
  lmm3 <- lme(Y3 ~ eta2 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
  lmm4 <- lme(Y4 ~ eta3 - 1, random = ~ 1 | user.id,
              data = sim_dat, method = 'REML')
}
